...
def installLinuxHttpd(context,builder,currVendor,installScript) {
	...
	if ( context.isLocalCloud() ) {
		if ( context.attributes.thisApplication["installing"] == null || context.attributes.thisApplication["installing"] == false ) {
			...
			while ( context.attributes.thisApplication["installing"] == true ) {
				println "ApacheWeb_install.groovy: Waiting for apt-get/yum (on localCloud) to end on another service instance in this application... "
				sleep 10000			
			}
		}
	}
	...
	builder.sequential {
		...
		echo(message:"ApacheWeb_install.groovy: Running ${context.serviceDirectory}/${installScript} os is ${currVendor}...")
	...
	if ( context.isLocalCloud() ) {
		...
		println "ApacheWeb_install.groovy: Finished using apt-get/yum on localCloud"
	}	
}

//def installWindowsHttpd(config,osConfig,downloadFile,builder) {
//	builder.sequential {
//			echo(message:"ApacheWeb_install.groovy: installing on Windows...")
//			mkdir(dir:"install")
